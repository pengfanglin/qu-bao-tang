...
import com.fanglin.model.banner.AdminBannerModel;
...
public class SystemServiceImpl implements SystemService {
     ...
     */
    @Override
    public int updateBanner(AdminBannerModel bannerModel) {
        this.writeBannerHtml(bannerModel);
        bannerModel.setContent(null);
        return mapperFactory.bannerMapper.updateByPrimaryKeySelective(BeanUtils.copy(bannerModel,BannerEntity.class));
    }
     ...
     */
    @Override
    public int insertBanner(AdminBannerModel bannerModel) {
        this.writeBannerHtml(bannerModel);
        bannerModel.setContent(null);
        return mapperFactory.bannerMapper.insertSelective(BeanUtils.copy(bannerModel,BannerEntity.class));
    }
     ...
     * @param banner
     */
    private void writeBannerHtml(AdminBannerModel banner) {
        if ("common".equals(banner.getBannerType()) && banner.getContent() != null) {
            HtmlStyleEntity htmlStyle = this.getHtmlStyle(new HtmlStyleEntity().setStyleType("common"));
            if (htmlStyle == null) {
                throw new ValidateException("html样式不能为空");
            }
            if (banner.getBannerUrl() == null || "".equals(banner.getBannerUrl())) {
                ...
                boolean result = OthersUtils.writeHtml(path, banner.getContent(), htmlStyle.getStyleDesc());
                if (result) {
                    banner.setBannerUrl(path);
                }
            } else {
                OthersUtils.writeHtml(banner.getBannerUrl(), banner.getContent(), htmlStyle.getStyleDesc());
            }
        }
    }
